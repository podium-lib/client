...
module.exports = state =>
    new Promise((resolve, reject) => {
        if (state.status === 'empty') {
            state.emit(
                'warn',
                'no manifest available - can not fetch main content - serving fallback'
            );
            state.success = true;
            state
                .fallbackStream(() => {
        ...
        state.emit(
            'debug',
            `start reading main content from remote resource - manifest version is ${state
                .manifest.version}`
        );
        ...
        r.on('response', response => {
            ...
            state.emit(
                'debug',
                `got head response from remote resource for main content - header version is ${response
            ...
            if (
                ...
                state.emit(
                    'info',
                    `podlet version number in header differ from cached version number - aborting request to remote resource for main content`
                );
                r.abort();
                state.status = 'stale';
                return;
