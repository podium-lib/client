    ...
    resolve(outgoing) {
        return new Promise((resolve, reject) => {
            ...
            r.on('response', response => {
                ...
                const resError = outgoing.statusCode !== 200;
                if (resError && outgoing.throwable) {
                    ...
                    this.log.warn(
                        `remote resource responded with non 200 http status code for content - code: ${outgoing.statusCode} - resource: ${outgoing.name} - url: ${outgoing.contentUri}`,
                    );

                    reject(
                        boom.boomify(
                            new Error(
                                `Could not read content. Resource responded with ${outgoing.statusCode} on ${outgoing.contentUri}`,
                            ),
                            {
